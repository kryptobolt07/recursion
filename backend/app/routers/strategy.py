...
async def strategy_report(channel_id: str, force: bool = False):
    """§5.1 Channel Strategy Report — niche mix reco + 30/60/90 roadmap."""
    try:
        return await service.strategy_report(channel_id, force=force)
...
async def generate_video_ideas(channel_id: str, force: bool = False):
    """§5.2 Video Idea Generator — 10-20 ideas with rationale and urgency tags."""
    try:
        return await service.video_ideas(channel_id, force=force)
    except Exception as exc:
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=502, detail=str(exc)) from exc
...
async def posting_strategy(channel_id: str, force: bool = False):
    """§5.5 Video Posting Strategy — best day/time, weekly calendar, Shorts reco."""
    try:
        return await service.posting_strategy(channel_id, force=force)
    except Exception as exc:
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=502, detail=str(exc)) from exc
